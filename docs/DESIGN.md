...
The composition it exists to make is literal: **the office is green, the paper is white.** The record card sits on the field as a document on a registrar's desk, and the contrast between the drenched field and the white record *is* the design — not a treatment applied to it. The field is ruled with faint horizontal lines at 6% white, the way a register is ruled. That texture is the one permitted flourish, and it is a real records-office object, not costume: it must never drift toward aged paper, sepia, or a deckled edge.
